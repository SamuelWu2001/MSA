...
def get_decath_loader(args):
    ...
    class_list = [2,3,6,7,8,9,10] # 2,3,6,7,8,9,10
    train_list = []
    valid_list = []

    for i in range(len(class_list)) :
        split_JSON = f"dataset_{class_list[i]}.json"
        datasets = os.path.join(data_dir, split_JSON)
        datalist = custom_load_decathlon_datalist(datasets, "training")
        split_index = len(datalist) * 9 // 10
        train_list.extend(datalist[:split_index])
        valid_list.extend(datalist[split_index:])
    ...
    train_eval_loader = DataLoader(train_eval_ds, num_workers=0, batch_size=args.b, shuffle=True)
    val_eval_ds = CacheDataset(
        data=valid_list, transform=eval_transforms, cache_num=6, cache_rate=1.0, num_workers=0
    )
    val_eval_loader = ThreadDataLoader(val_eval_ds, num_workers=0, batch_size=1, shuffle=False)
